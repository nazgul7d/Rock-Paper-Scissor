...
let player = 0
let computer = 0
// get the first element with class="roundResult"
...
function playGame(playerChoice) {
    ...
    if (player < 5 && computer < 5) {
        const combos = [["rock", "scissor"], ["paper", "rock"], ["scissor", "paper"]]
        ...
        if (playerChoice === computerChoice) {
            ...
            for (const i of combos) {
                if (playerChoice === i[0] && computerChoice ===  i[1]) {
                    ...
                    ++computer
                    roundResult.textContent = `You lose! ${capitalizeWord(i[0])} beats ${capitalizeWord(i[1])}. Cool cool.`
                    updateScore("c")
                }
            }
        }
        ...
        roundResult.textContent = 'You won, congrats!!!'
    } else if (computer === 5) {
